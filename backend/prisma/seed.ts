...
const REST: Seed[] = [
  ...
  },
  {
    ...
    bodyMd: [
      "Is the Tavernmaster (Front of House) of [[The Everwyvern House]], the fancier inn in [[Triboar]] — mostly Waterdhavian nobles.",
      "",
  ...
  },
  {
    ...
    bodyMd:
      "Where [[Pendrel Dornwood]] introduced the party to [[Krag Bronzebeard]].",
  },
  {
    type: "location",
    ...
    knowledge: "known",
    tags: ["triboar", "inn"],
    data: { kind: "Inn", within: "[[Triboar]]", ruledBy: "Unknown" },
    bodyMd:
  ...
  },
  {
    ...
    data: {
      kind: "Humanoid",
      habitat: "Caves and camps",
      groupSize: "Leads goblin bands",
    },
    bodyMd: [
      "## What is commonly known",
  ...
  },
  {
    ...
    data: {
      kind: "Humanoid",
      habitat: "Trade roads",
      groupSize: "Gangs with a captain",
    },
    bodyMd: [
      "## What is commonly known",
...
async function main() {
  ...
  const holdings = REST.filter(
    (s) => s.type === "item" && s.quantity !== undefined,
  ).map((s) => ({
  ...
  console.log(
    `seeded ${entities} entities, ${held} holdings and ${notes} notes`,
  );
...
async function seedNotes(playerByName: Map<string, string>) {
  ...
  const byName = new Map(
    (await prisma.entity.findMany({ select: { id: true, name: true } })).map(
      (e) => [e.name, e.id],
    ),
  );
